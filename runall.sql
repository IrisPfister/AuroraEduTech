...
SELECT
    ...
    t.nome_turma,
    t.ano_escolar,
    sm.descricao  AS status_matricula
FROM tb_matricula       m
...
JOIN tb_status_matricula sm ON sm.pk_status_matricula = m.fk_status_matricula
WHERE sm.descricao = 'ativa'
ORDER BY t.ano_escolar, t.nome_turma, p.sobrenome;
...
SELECT
    ...
    t.ano_escolar,
    t.periodo,
    COUNT(m.pk_matricula)  AS total_alunos,
    t.capacidade_maxima,
    ROUND(COUNT(m.pk_matricula) * 100.0
          / NULLIF(t.capacidade_maxima, 0), 1)  AS pct_ocupacao
FROM tb_turma            t
JOIN tb_ano_letivo       al ON al.pk_ano_letivo = t.fk_ano_letivo
JOIN tb_matricula        m  ON m.fk_turma       = t.pk_turma
JOIN tb_status_matricula sm ON sm.pk_status_matricula = m.fk_status_matricula
WHERE al.ativo = TRUE
  AND sm.descricao = 'ativa'
GROUP BY t.pk_turma, t.nome_turma, t.ano_escolar, t.periodo, t.capacidade_maxima
ORDER BY t.ano_escolar, t.nome_turma;
...
SELECT
    ...
    b.bimestre,
    ROUND(
        SUM(na.valor_nota * ap.peso) / NULLIF(SUM(ap.peso), 0)
    , 2)  AS media_ponderada
FROM tb_boletim              b
JOIN tb_matricula            m   ON m.pk_matricula           = b.fk_matricula
JOIN tb_aluno                a   ON a.pk_aluno               = m.fk_aluno
JOIN tb_pessoa               p   ON p.pk_pessoa              = a.fk_pessoa
JOIN tb_turma                t   ON t.pk_turma               = m.fk_turma
JOIN tb_nota_atividade       na  ON na.fk_matricula          = m.pk_matricula
JOIN tb_atividade_professor  ap  ON ap.pk_atividade_professor = na.fk_atividade_professor
                                 AND ap.bimestre              = b.bimestre
WHERE b.bimestre = (
        SELECT MAX(b2.bimestre)
        FROM tb_boletim b2
        WHERE b2.fk_matricula = b.fk_matricula
    )
  AND na.valor_nota IS NOT NULL
GROUP BY b.pk_boletim, p.nome, p.sobrenome, a.ra, t.nome_turma, b.bimestre
HAVING media_ponderada < 5
ORDER BY media_ponderada;
 
 
SELECT
    p_aluno.nome                         AS aluno_nome,
    p_aluno.sobrenome                    AS aluno_sobrenome,
    a.ra,
    t.nome_turma,
    pg.data_vencimento,
    ...
    p_resp.sobrenome                     AS responsavel_sobrenome,
    e.email                              AS email_responsavel
FROM tb_pagamento           pg
JOIN tb_status_pagamento    sp   ON sp.pk_status_pagamento     = pg.fk_status_pagamento
JOIN tb_contrato_financeiro cf   ON cf.pk_contrato_financeiro  = pg.fk_contrato_financeiro
JOIN tb_matricula           m    ON m.pk_matricula             = cf.fk_matricula
JOIN tb_aluno               a    ON a.pk_aluno                 = m.fk_aluno
JOIN tb_pessoa              p_aluno ON p_aluno.pk_pessoa       = a.fk_pessoa
JOIN tb_turma               t    ON t.pk_turma                 = m.fk_turma
JOIN tb_responsavel_aluno   ra   ON ra.fk_aluno                = a.pk_aluno
                                 AND ra.responsavel_financeiro  = TRUE
JOIN tb_responsavel         r    ON r.pk_responsavel           = ra.fk_responsavel
JOIN tb_pessoa              p_resp ON p_resp.pk_pessoa         = r.fk_pessoa
LEFT JOIN tb_email          e    ON e.pk_email = (
        SELECT pk_email
        FROM tb_email
        WHERE fk_pessoa = r.fk_pessoa
          AND principal = TRUE
        LIMIT 1
    )
WHERE sp.descricao        != 'pago'
  AND sp.descricao        != 'cancelado'
  AND pg.data_vencimento  < CURDATE() - INTERVAL 5 DAY
...
WITH frequencia AS (
    SELECT
        ...
        t.pk_turma,
        t.nome_turma,
        COUNT(pa.pk_presenca)                      AS total_aulas,
        SUM(CASE WHEN pa.presenca = TRUE THEN 1 ELSE 0 END) AS presencas,
        ROUND(
            SUM(CASE WHEN pa.presenca = TRUE THEN 1 ELSE 0 END) * 100.0
            / NULLIF(COUNT(pa.pk_presenca), 0)
        , 2)                                        AS pct_frequencia
    FROM tb_matricula       m
    JOIN tb_aluno           a   ON a.pk_aluno  = m.fk_aluno
    JOIN tb_pessoa          p   ON p.pk_pessoa = a.fk_pessoa
    JOIN tb_turma           t   ON t.pk_turma  = m.fk_turma
    JOIN tb_presenca_aluno  pa  ON pa.fk_matricula = m.pk_matricula
    JOIN tb_ano_letivo      al  ON al.pk_ano_letivo = t.fk_ano_letivo
    WHERE al.ativo = TRUE
    GROUP BY m.pk_matricula, p.nome, p.sobrenome, a.ra, t.pk_turma, t.nome_turma
)
SELECT
    nome_turma,
    nome,
    ...
    presencas,
    pct_frequencia,
    RANK() OVER (PARTITION BY pk_turma ORDER BY pct_frequencia DESC) AS rank_turma
FROM frequencia
ORDER BY nome_turma, rank_turma;
...
CREATE PROCEDURE sp_registrar_pagamento(
    IN p_contrato_fin  INT,
    IN p_mes           TINYINT,
    IN p_ano           YEAR,
    IN p_valor_pago    DECIMAL(10,2)
)
BEGIN
    DECLARE v_pk_pago      INT;
    DECLARE v_pk_pagamento INT;
    DECLARE v_rows         INT;

    START TRANSACTION;

        SELECT pk_status_pagamento INTO v_pk_pago
        FROM tb_status_pagamento
        WHERE descricao = 'pago'
        LIMIT 1;

        SELECT pk_pagamento INTO v_pk_pagamento
        FROM tb_pagamento
        WHERE fk_contrato_financeiro = p_contrato_fin
        ...
        UPDATE tb_pagamento
        SET
            fk_status_pagamento = v_pk_pago,
            valor_pago          = p_valor_pago,
            data_pagamento      = CURDATE()
        WHERE pk_pagamento = v_pk_pagamento;

        SET v_rows = ROW_COUNT();

        IF v_rows = 1 THEN
            COMMIT;
            SELECT 'Pagamento registrado com sucesso' AS resultado, v_pk_pagamento AS pk_pagamento;
        ELSE
            ROLLBACK;
            SELECT 'Nenhum pagamento localizado — operacao revertida' AS resultado, NULL AS pk_pagamento;
        END IF;

END//
DELIMITER ;

-- Substitua os valores abaixo pelos IDs reais antes de executar:
-- CALL sp_registrar_pagamento(p_contrato_fin, p_mes, p_ano, p_valor_pago);
CALL sp_registrar_pagamento(1, 5, 2025, 950.00);
 
 
 
 
CREATE INDEX idx_pagamento_vencimento_status
    ON tb_pagamento (data_vencimento, fk_status_pagamento);
 
CREATE INDEX idx_pagamento_ano_mes
    ON tb_pagamento (ano_referencia, mes_referencia);
 
CREATE INDEX idx_folha_ano_mes_status
    ON tb_folha_pagamento (ano, mes, status_folha);
 
CREATE INDEX idx_presenca_matricula_cal
    ON tb_presenca_aluno (fk_matricula, fk_calendario_letivo);
 
CREATE INDEX idx_nota_atividade
    ON tb_nota_atividade (fk_atividade_professor, valor_nota);
 
CREATE INDEX idx_boletim_matricula_bimestre
    ON tb_boletim (fk_matricula, bimestre);
 
CREATE INDEX idx_matricula_turma_status
    ON tb_matricula (fk_turma, fk_status_matricula);
 
CREATE INDEX idx_alerta_status_data
    ON tb_alerta_evasao (fk_status_alerta, data_alerta);
 
 
EXPLAIN SELECT pk_pagamento, data_vencimento, valor_original
FROM tb_pagamento IGNORE INDEX (idx_pagamento_vencimento_status)
WHERE fk_status_pagamento = 2             -- status "pendente"
  AND data_vencimento < CURDATE() - INTERVAL 5 DAY;
 
EXPLAIN SELECT pk_pagamento, data_vencimento, valor_original
FROM tb_pagamento
WHERE fk_status_pagamento = 2
  AND data_vencimento < CURDATE() - INTERVAL 5 DAY;
 
 
EXPLAIN SELECT fk_matricula, COUNT(*) AS total, SUM(presenca) AS presentes
FROM tb_presenca_aluno IGNORE INDEX (idx_presenca_matricula_cal)
WHERE fk_matricula = 42
GROUP BY fk_matricula;
 
EXPLAIN SELECT fk_matricula, COUNT(*) AS total, SUM(presenca) AS presentes
FROM tb_presenca_aluno
WHERE fk_matricula = 42
GROUP BY fk_matricula;
...
/*  ← remova este bloco de comentário para executar
 
SET FOREIGN_KEY_CHECKS = 0;
 
TRUNCATE TABLE tb_verba_folha;
TRUNCATE TABLE tb_folha_pagamento;
TRUNCATE TABLE tb_pagamento;
TRUNCATE TABLE tb_contrato_financeiro;
TRUNCATE TABLE tb_nota_atividade;
TRUNCATE TABLE tb_boletim;
TRUNCATE TABLE tb_presenca_aluno;
TRUNCATE TABLE tb_alerta_evasao;
TRUNCATE TABLE tb_atividade_professor;
TRUNCATE TABLE tb_horario_aula;
TRUNCATE TABLE tb_grade_curricular;
TRUNCATE TABLE tb_matricula;
TRUNCATE TABLE tb_responsavel_aluno;
TRUNCATE TABLE tb_profissao_responsavel;
TRUNCATE TABLE tb_responsavel;
TRUNCATE TABLE tb_formacao_professor;
TRUNCATE TABLE tb_professor;
TRUNCATE TABLE tb_historico_salario;
TRUNCATE TABLE tb_historico_cargo;
TRUNCATE TABLE tb_contrato;
TRUNCATE TABLE tb_funcionario;
TRUNCATE TABLE tb_aluno;
TRUNCATE TABLE tb_evento_escolar;
TRUNCATE TABLE tb_calendario_letivo;
...
TRUNCATE TABLE tb_area_conhecimento;
TRUNCATE TABLE tb_materia;
TRUNCATE TABLE tb_cargo;
TRUNCATE TABLE tb_tipo_contrato;
TRUNCATE TABLE tb_tipo_atividade;
TRUNCATE TABLE tb_tipo_aula;
TRUNCATE TABLE tb_tipo_ingresso;
TRUNCATE TABLE tb_tipo_escola_anterior;
TRUNCATE TABLE tb_tipo_risco_evasao;
TRUNCATE TABLE tb_tipo_verba;
TRUNCATE TABLE tb_nivel_formacao;
TRUNCATE TABLE tb_nivel_risco_evasao;
TRUNCATE TABLE tb_situacao_boletim;
TRUNCATE TABLE tb_status_alerta;
TRUNCATE TABLE tb_status_matricula;
TRUNCATE TABLE tb_status_pagamento;
TRUNCATE TABLE tb_profissao;
TRUNCATE TABLE tb_ano_letivo;
...
DROP DATABASE IF EXISTS sistema_aurora_edutech;
